...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    /*
     * 1. 使用广度优先算法
     * 2. 如果水的初始高度低于地形，不会造成任何的影响
     */
    Queue<GridLocation> waterQueue;
    // whether it’s under water (true) or above the water (false).
    int terrainRow = terrain.numRows();
    int terrainCol = terrain.numCols();
    Grid<bool> resultWaterGrid(terrainRow, terrainCol, 0);

    for (const GridLocation& waterSource: sources) {
        if (waterSource.row < terrainRow && waterSource.col < terrainCol && waterSource.row >= 0 && waterSource.col >= 0) {
            if (height > terrain.get(waterSource.row, waterSource.col)) {
                waterQueue.enqueue(waterSource);
                resultWaterGrid.set(waterSource.row, waterSource.col, 1);
            }
        }
    }
    ...
    while (!waterQueue.isEmpty()) {
        GridLocation tempLocation = waterQueue.dequeue();
        for (int i = 0; i < 4; i++) {
            int x = dx[i] + tempLocation.row;
            int y = dy[i] + tempLocation.col;
            // 进行越界检查
            if (x > -1 && y > -1 && x < terrainRow && y < terrainCol) {
                if (height > terrain.get(x, y)) {
                    // 检查是否有重复的入队
                    if (!resultWaterGrid.get(x, y)) {
                        GridLocation newWaterSource;
                        newWaterSource.row = x;
                        newWaterSource.col = y;
                        waterQueue.enqueue(newWaterSource);
                        resultWaterGrid.set(newWaterSource.row, newWaterSource.col, 1);
                    }
                }
            }
        }
