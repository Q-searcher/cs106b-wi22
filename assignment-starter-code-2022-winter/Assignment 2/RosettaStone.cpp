...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength <= 0) {
        error("the kGramLength parameter should be positive ");
    }
    int stringLength = str.length();
    // 检测kGramLength的长度是否大于文本的长度
    if (stringLength < kGramLength) {
    ...
    string tempString;
    double tempValue;
    for (int i = 0; i < stringLength - kGramLength + 1; i++) {
        tempString = str.substr(i, kGramLength);
        if (map.containsKey(tempString)) {
            tempValue = map.get(tempString);
            map.put(tempString, tempValue + 1);
        }
        else {
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    sum = sqrt(sum);
    for (const string& key: input) {
        double tempValue = input.get(key);
        tempValue = tempValue / sum;
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    /*
     * 1. 使用error处理numTokeep是负数的情况
     * 2. 如果numTokeep大于键值对的数量，返回所有键值对
     * 3. The element that’s returned by dequeue() or peek() is the element that has the lowest weight
     */
    // check whether numTokeep is a positive number
    int size = source.size();
    PriorityQueue<string> queue;
    Map<string, double> newMap;
    if (numToKeep < 0) {
        error("numTokeep should not to be negative");
    }
    ...
    // 将键值对加载到PriorityQueue中
    for (const string& key: source) {
        queue.enqueue(key, source.get(key));
    }
    ...
    for (int i = 0; i < numToKeep; i++) {
        double tempValue = queue.peekPriority();
        string tempKey = queue.dequeue();
        newMap.put(tempKey, tempValue);
    }
    return newMap;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    // 检测corpora是否为空
    if (corpora.isEmpty()){
        error("corpora should not to be empty");
    }
    // 遍历set中的所有map，计算和textProfile的cosineSimlarity
    Map<string, double> cosineSimlarityMap;
    for (const Corpus& tempCorpora: corpora) {
        string tempString;
        double consineSimlarityValue;

        consineSimlarityValue = cosineSimilarityOf(textProfile, tempCorpora.profile);
        tempString = tempCorpora.name;
        cosineSimlarityMap.put(tempString, consineSimlarityValue);

    }
    double biggestConsineSimlarity = 0;
    string resultString;
    for (const string& key: cosineSimlarityMap) {
        if (biggestConsineSimlarity < cosineSimlarityMap.get(key)) {
            biggestConsineSimlarity = cosineSimlarityMap.get(key);
            resultString = key;
        }
    }
    return resultString;
